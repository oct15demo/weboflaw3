...
def score_party_similarities(party1a,party2a,party1b,party2b,trace=False):
    if trace:
        print(1,party1a,2,party2a,3,party1b,4,party2b)
    ...
    if (len(party2b)>=len(party2a)):
        ...
        if (party2b in party2a):
            ...
            party2_score = 0
    if trace:
        print('5!!!!!',party1_score,party2_score)
    return((party1_score+party2_score)/2)

def increment_party_key(party_key,info,trace=False):
    global global_citations
    global party_key_hash
    out_key = False
    if party_key in party_key_hash:
        if trace:
            print('in pkh:',party_key_hash[party_key],end='')
        if 'id1' in info:
            id1 = info['id1']
        else:
            id1 = False
        ...
        for other_party_key in party_key_hash[party_key]:
            ...
            if trace:
                print('in opkh:',other_entry)
                print('id1',id1)
                print('full',full_party1,full_party2)
            ...
            elif (not id1) and full_party1 and full_party2 and ('party1' in other_entry) and ('party2' in other_entry):
                if trace:
                    print('*2*',full_party1,other_entry,other_party_key,end='')
                party_score = score_party_similarities(full_party1.lower(),full_party2.lower(),other_entry['party1'].lower(),other_entry['party2'].lower(),trace=trace)
                if trace: 
                    print('2.01')
                possible_keys.append([party_score,other_party_key])
                if trace:
                    print('*2.1')
            else:
                different_key = True
        ...
        if out_key:
            ...
            if trace:
                print('-add_one_to_party_start-',end='')
            out_key = add_one_to_party_key(party_key_hash[party_key][-1])
            if trace:
                print('-add_one_to_party_end-',end='')
            out_key = add_one_to_party_key(party_key_hash[party_key][-1])
        if trace:
            print('*4.5')
...
def score_case_similarities(casename1,casename2,trace=False):
    if trace:
        print('score_case1',casename1,casename2)
    word_list1 = []
    word_list2 = []
    ...
    if trace:
        print('score_case2',length)
    for word in word_list1:
        if word in word_list2:
            overlap_length += 1
            word_list2.remove(word)
    if trace:
        print('score_case3',overlap_length,length)
    if length>0:
        return(overlap_length/length)
    elif (len(casename1)>0) and (casename1 == casename2):
        return(1)
    else:
        return(0)

def increment_case_key(case_key,info,trace=False):
    global global_citations
    global party_key_hash
    out_key = False
    if case_key in party_key_hash:
        if trace:
            print('inc_case_key',1,case_key,party_key_hash[case_key])
        if 'id1' in info:
            id1 = info['id1']
        else:
            id1 = False
        ...
        different_key = False
        if trace:
            print('inc_case_key',1.1)
        for other_case_key in party_key_hash[case_key]:
            if trace:
                print('other',other_case_key)
            if other_case_key in global_citations:
                other_entry = global_citations[other_case_key]
            else:
                other_entry = False
            if trace:
                print(2.1,other_entry)
                print('id1',id1)
            if not other_entry:
                if trace:
                    print('2.1.1')
                pass
            elif out_key or different_key:
                if trace:
                    print('2.1.2')
                pass
            elif id1 and ('id1' in other_entry):
                ...
                if 'casename' in info:
                    ...
                    name2 = False
                if trace:
                    print('2.1.5',name1,name2,possible_keys)
                if (not name1):
                    ## possible if entered as an alternate key only
                    possible_keys.append([score_case_similarities(case_key,other_case_key,trace=trace),other_case_key])
                else: 
                    possible_keys.append([score_case_similarities(name1.lower(),name2.lower(),trace=trace),other_case_key])
            else:
                different_key = True
        if trace:
            print(3.1)
...
def generate_key_words_from_case_record(info,trace=False):
    ...
    if trace:
        print('d-->',end='')
        print("party1",party1,"party2",party2,"casename",casename,end='')
    if party1 and party2:
        party_key = party_key = re.sub('[ ,\.]','',(party1+party2).lower())
        if trace:
            print('---i---party_key',party_key,end='')
        party_key = increment_party_key(party_key,info,trace=trace)
        if trace:
            print('---ii---party_key',party_key,end='')
        ...
        if len(casename)>20:
            ...
            # if trace:
            #     print('short casename:',casename)
            case_key = standardize(casename).lower() 
        if trace:
            print('incrementing trace name')
            case_key = increment_case_key(case_key,info,trace=True)
        else:
            case_key = increment_case_key(case_key,info)
        if trace:
            print('created case_key',case_key)
...
def create_csv_file(output_file,input_directories,IE_infile_type,outfile_prefix,trace=False):
